...
namespace Codecov.Services.Url
{
    public class QueryTests
    {
        ...
        public void Multiple_Repositories_Should_Used_ContiniousIntegrationServer_First()
        {
            ...
            var continiousIntegrationServer = Substitute.For<IRepositoryService>();
            continiousIntegrationServer.Branch.Returns("develop");
            var versionControlSystem = Substitute.For<IRepositoryService>();
            versionControlSystem.Branch.Returns("dev");
            var build = Substitute.For<ContinuousIntegrationService>();
            var yaml = Substitute.For<IYamlService>();
            var query = new Query(queryOptions, new[] { continiousIntegrationServer, versionControlSystem }, build, yaml);

            // When
            var getQuery = query.Value.Split('&');
        ...
        public void Should_Return_Defaults()
        {
            // Given
            var queryOptions = Substitute.For<IQueryOptions>();
            var repository = Substitute.For<IEnumerable<IRepositoryService>>();
            var build = Substitute.For<ContinuousIntegrationService>();
            var yaml = Substitute.For<IYamlService>();
            ...
            // Then
            getQuery.FirstOrDefault(x => x.StartsWith("package=")).Should().Be($"package={Extensions.Version}");
        }
        ...
        public void Should_Escape_Hash_Token_In_Branch()
        {
            // Given
            var queryOptions = Substitute.For<IQueryOptions>();
            var repository = Substitute.For<IRepositoryService>();
